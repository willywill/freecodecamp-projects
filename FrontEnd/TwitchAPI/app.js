...
const fakeFetch = async () => {
  ...
  const reduceAPIResponse = async (res) => {
    res = res.filter(x => !x.error); 
    return res.map(data => { 
      if (data.stream) {
        return {
          displayName: data.stream.display_name,
          status: data.stream.status,
          game: data.stream.game,
          link: `https://www.twitch.tv/${data._links.self.split("https://api.twitch.tv/kraken/streams/")[1]}`,
          logo: data.stream.logo
        }  
      } else {
        return {
          displayName: data.display_name,
          status: 'This streamer is currently offline.',
          game: '',
          link: `https://www.twitch.tv/${data._links.self.split("https://api.twitch.tv/kraken/streams/")[1]}`,
          logo: ''
        }
  ...
  const app = new Vue ({
    ...
    async created () {
      const res = await getTwitchData();
      this.streams = await reduceAPIResponse(res);
    }
  });